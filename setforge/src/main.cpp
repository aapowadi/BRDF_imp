...
void InitRenderer(Arguments& opt)
{
	...
	else if (opt.cam == POSE)
	{
		...
		if (opt.material == Fe) {
			pose_renderer->setMaterial("Fe");
		}
		else if (opt.material == WOOD) {
			pose_renderer->setMaterial("WOOD");
		}
		else if (opt.material == Cu) {
			pose_renderer->setMaterial("Cu");
		}
		else if (opt.material == Painted) {
			pose_renderer->setMaterial("Painted");
		}
